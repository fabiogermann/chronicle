...
class RetryHandlerTest {
    ...
    @Test
    fun `withRetry succeeds after one failure`() = runTest {
        var callCount = 0
        
        val result = withRetry(RetryConfig(maxAttempts = 3, initialDelayMs = 10)) {
            callCount++
            if (callCount == 1) {
    ...
    @Test
    fun `withRetry exhausts all attempts`() = runTest {
        var callCount = 0
    ...
    @Test
    fun `withRetry respects shouldRetry predicate`() = runTest {
        var callCount = 0
    ...
    @Test
    fun `withRetry invokes onRetry callback`() = runTest {
    ...
    @Test
    fun `withRetry calculates exponential backoff correctly`() = runTest {
    ...
    @Test
    fun `withRetry respects maxDelayMs cap`() = runTest {
        val delays = mutableListOf<Long>()
    ...
    @Test
    fun `withRetryOrThrow returns value on success`() = runTest {
        val result = withRetryOrThrow(RetryConfig(maxAttempts = 3, initialDelayMs = 10)) {
            "success"
        }
    ...
    @Test
    fun `withRetryOrThrow throws on failure`() = runTest {
    ...
    @Test
    fun `withRetry passes attempt number to block`() = runTest {
    ...
    @Test
    fun `RetryResult Success contains correct attempt number`() = runTest {
        var callCount = 0
        
        val result = withRetry(RetryConfig(maxAttempts = 5, initialDelayMs = 10)) {
            callCount++
            if (callCount < 3) {
                throw IOException("Fail")
            }
    ...
    @Test
    fun `RetryResult Failure contains correct attempts made`() = runTest {
        val result = withRetry(RetryConfig(maxAttempts = 4, initialDelayMs = 10)) {
            throw IOException("Always fails")
        }
