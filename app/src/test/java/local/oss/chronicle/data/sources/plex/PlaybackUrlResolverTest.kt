...
class PlaybackUrlResolverTest {
    ...
    private val testTrack = MediaItemTrack(
        id = 123,
        title = "Test Track",
     ...
     */
    @Test
    fun `resolveStreamingUrl returns cached URL when valid`() = runBlocking {
     ...
     */
    @Test
    fun `resolveStreamingUrl refreshes expired cache entries`() = runBlocking {
     ...
     */
    @Test
    fun `onServerUrlChanged invalidates cache`() = runBlocking {
        val expectedUrl = "$testServerUrl/transcode/session/abc123"
        setupSuccessfulResolution(testTrack, expectedUrl)
     ...
     */
    @Test
    fun `resolveStreamingUrl retries on network errors`() = runBlocking {
        val expectedUrl = "$testServerUrl/transcode/session/abc123"
     ...
     */
    @Test
    fun `resolveStreamingUrl gives up after max retry attempts`() = runBlocking {
     ...
     */
    @Test
    fun `preResolveUrls resolves multiple tracks in parallel`() = runBlocking {
        val tracks = listOf(
            testTrack,
     ...
     */
    @Test
    fun `preResolveUrls reports failed tracks`() = runBlocking {
     ...
     */
    @Test
    fun `cache invalidation notifies listeners`() = runBlocking {
        var listenerCalled = false
        val listener = object : PlaybackUrlResolver.OnCacheInvalidatedListener {
     ...
     */
    @Test
    fun `concurrent resolveStreamingUrl calls are thread safe`() = runBlocking {
     ...
     */
    @Test
    fun `preResolveUrls handles empty track list`() = runBlocking {
     ...
     */
    @Test
    fun `refreshUrlsOnNetworkChange resolves tracks`() = runBlocking {
        val tracks = listOf(
            testTrack,
            testTrack.copy(id = 124, media = "/library/parts/457/file.mp3")
        )
     ...
     */
    @Test
    fun `resolveStreamingUrl does not retry non-retryable errors`() = runBlocking {
     ...
     */
    @Test
    fun `onServerUrlChanged does not invalidate if URL unchanged`() = runBlocking {
        val expectedUrl = "$testServerUrl/transcode/session/abc123"
        setupSuccessfulResolution(testTrack, expectedUrl)
    ...
    private fun setupSuccessfulResolution(track: MediaItemTrack, expectedStreamUrl: String) {
    ...
    private fun createSuccessfulDecision(streamUrl: String): PlexTranscodeDecisionWrapper {
        ...
        val part = PlexTranscodePart(
            streamUrl = streamPath,
            decision = "directplay",
            selected = true
        )
        
        val media = PlexTranscodeMedia(
            parts = listOf(part),
            selected = true
        )
        
        val metadata = PlexTranscodeMetadata(
            media = listOf(media)
        )
        
        val container = PlexTranscodeDecision(
