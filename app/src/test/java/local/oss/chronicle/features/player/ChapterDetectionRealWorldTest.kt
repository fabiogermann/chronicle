...
class ChapterDetectionRealWorldTest {
    ...
    fun `test all chapter boundaries are correctly detected`() {
        ...
        assertThat(
            ...
            `is`("2")  // Inclusive range means 5197230 is start of chapter 2
        )
    ...
    fun `test edge case - position exactly at chapter end boundary`() {
        ...
        assertThat(
            ...
            atPrologueEnd.title,
            `is`("Part I: The Chasm")  // Because range is inclusive, end position belongs to next chapter
        )
     ...
     */
    private fun List<Chapter>.getChapterAtUsingBrokenCachedLogic(trackId: Long, progress: Long): Chapter {
